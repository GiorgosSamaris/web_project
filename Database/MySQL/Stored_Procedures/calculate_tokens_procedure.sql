...
SET tokens_pool = ROUND((tokens_pool * 0.8), 0)*100;
SELECT SUM(current_score) FROM customer INTO score_sum;
UPDATE customer SET overall_tokens = overall_tokens + ROUND(((current_score/score_sum)*tokens_pool),0), current_score = 0 WHERE current_score > 0;
UPDATE customer SET last_months_tokens = ROUND(((current_score/score_sum)*tokens_pool),0), current_score = 0 WHERE current_score > 0;
END$
DELIMITER ;
